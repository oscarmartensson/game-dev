...
#define GLM_ENABLE_EXPERIMENTAL     // To use for hashing of the Vertex struct.
#include <glm/glm.hpp>
...
struct Vertex {
  ...
  bool operator==( const Vertex& other ) const {
    // Same if pos, color and texcoord is the same.
...
// To use for flat single texture, statue.jpg
...
private:
  ...
  void updateUniformBuffers( const uint32_t imageIndex) {
    ...
    const float rotationSpeedFactor = 0.1f;

    UniformBufferObject ubo = {};
    // Rotate the model around the z-axis 90 degress as time progresses.
    ubo.model = glm::rotate( glm::mat4( 1.0f ), time * rotationSpeedFactor * glm::radians( 90.0f ), glm::vec3( 0.0f, 0.0f, 1.0f ) );
  ...
  // Creates a texture sampler to use for texturing.
  ...
  // Creates image view used as depth image.
  ...
  // Loads the .obj model provided at the MODEL_PATH.
  void loadModel() {
    ...
    // Iterate over all faces to collect the indices and put in the member variable.
    for( const auto& shape : shapes ) {
      for( const auto& index : shape.mesh.indices ) {
        ...
        vertex.texCoord = {
          ...
          // We send UV coords to the GPU (Vulkan) where the vertical coord 1.0 is interpreted as bottom.
          // Flip to compensate for this.
